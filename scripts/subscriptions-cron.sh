...
# crontab стоит прямой curl к эндпоинту — таблица в CLAUDE.md. Именно эта строка
# установки, скопированная в crontab, и держала рассылку мёртвой с первого дня:
# cron каждые пять минут писал в лог `not found`, а задание не отработало ни
# разу. Скрипт оставлен для ручного запуска с локальной машины.
#
...
#
# Расписание и SEND_COOLDOWN_MS (src/lib/savedSearches.ts) — парные ручки. Если
# менять частоту прогона, кулдаун обязан остаться МЕНЬШЕ периода: он отсекает
# подписку до выборки, и при равных значениях крон будет отрабатывать вхолостую,
# ничего не отправляя.
#
