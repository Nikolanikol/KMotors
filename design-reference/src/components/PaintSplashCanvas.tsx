...
export default function PaintSplashCanvas() {
  ...
  useEffect(() => {
    ...
    // Draw brush stroke background pattern
    ...
    const animate = () => {
      ...
      ripplesRef.current = ripplesRef.current.filter((r) => {
        ...
        // Multiple concentric rings for organic feel
        for (let i = 0; i < 3; i++) {
        ...
        glowGrad.addColorStop(0, `rgba(255, 107, 26, ${r.opacity * 0.1})`)
      ...
      if (!isTouchDevice && mouseRef.current.x > 0) {
        ...
        const mouseGrad = ctx.createRadialGradient(mx, my, 0, mx, my, 40)
        mouseGrad.addColorStop(0, 'rgba(255, 69, 0, 0.15)')
