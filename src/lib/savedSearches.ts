...
/**
 * Не чаще одного сообщения на человека за этот срок.
 *
 * ⚠️ 20 часов, а НЕ 24, и это не небрежность. Крон ходит раз в сутки в
 * фиксированное время и сам по себе является ограничителем частоты; кулдаун —
 * лишь страховка от повторного запуска. Ровно 24 часа превращают его во второй
 * ограничитель, который дерётся с первым: отправка всегда происходит на
 * несколько минут ПОЗЖЕ старта крона (сходить в Encar, потом по карточкам),
 * поэтому на следующие сутки в момент выборки суток ещё не прошло, подписка
 * выпадает из listDueSubscriptions и не может сработать два дня подряд.
 *
 * Так подписка id=2 пропустила 30.07.2026: отправка в 15:30, следующий крон в
 * 11:00 — 19,5 часа вместо 24. Двадцать даёт запас на время обработки и сдвиг
 * расписания, но повторный прогон в тот же день по-прежнему отсекает.
 */
export const SEND_COOLDOWN_MS = 20 * 60 * 60 * 1000;
...
export async function saveSubscription(opts: {
  ...
  // Засев пуст, хотя Encar ответил. Само по себе это законно — на редкую машину
  // совпадений может не быть, — но точно так же выглядит и кривой запрос:
  // невалидную грамматику Encar отдаёт не ошибкой, а HTTP 200 с Count 0.
  // Подписку всё равно создаём (терять лид хуже), но случай должен быть виден в
  // логах, а не только по жалобе «мне ничего не приходит».
  if (seedIds.length === 0) {
    console.warn("[savedSearches] пустой засев, проверить запрос:", query);
  }
