 ...
 *
 * ⚠️ СЕЙЧАС СТОИТ ТЕСТОВОЕ ЗНАЧЕНИЕ (5 минут) под прогон крона каждые 5 минут.
 * Боевое значение — 20 часов, вернуть после тестов вместе с расписанием
 * `0 11 * * *` в crontab на VPS.
 *
 * Почему боевое 20 часов, а не 24: крон ходит раз в сутки в фиксированное время
 * и сам по себе является ограничителем частоты, а кулдаун — лишь страховка от
 * повторного запуска. Ровно 24 часа превращают его во второй ограничитель,
 * который дерётся с первым: отправка всегда происходит на несколько минут ПОЗЖЕ
 * старта крона (сходить в Encar, потом по карточкам), поэтому на следующие сутки
 * в момент выборки суток ещё не прошло, и подписка выпадает через раз. Ровно
 * так подписка id=2 пропустила 30.07.2026: отправка в 15:30, следующий крон в
 * 11:00 — 19,5 часа вместо 24.
 */
export const SEND_COOLDOWN_MS = 5 * 60 * 1000;
