 ...
 *
 * ⚠️ Две JSX-директивы в первых строках файла — не украшение. Файл собирают
 * ДВА разных инструмента: Next (там `jsx: "preserve"`, он всё делает сам) и
 * esbuild внутри tsx, когда `scripts/build-og-cards.tsx` импортирует отсюда
 * карточки. Без них esbuild берёт классический трансформ, зовёт
 ...
 *
 * ⚠️⚠️ Имена этих директив НЕЛЬЗЯ писать в комментариях текстом. esbuild ищет
 * их в ЛЮБОМ комментарии файла и берёт следующее слово как источник импорта:
 * фраза «директивы <имя> сверху» дала попытку загрузить модуль
 * `сверху/jsx-runtime` и падение сборки (12.08.2026). Описывать словами.
 *
