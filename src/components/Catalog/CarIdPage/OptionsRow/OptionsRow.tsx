  ...
  const groupOptions = (options: any[]) => {
    ...
    options.forEach((option) => {
      ...
      if (
        ...
        groups["Безопасность"].push(option);
      } else if (
        name.includes("аудио") ||
        name.includes("музыка") ||
        name.includes("звук")
      ) {
        groups["Развлечения"].push(option);
      } else if (
        ...
        groups["Технологии"].push(option);
      } else {
        groups["Другое"].push(option);
      }
    });
  ...
  return (
    <div className="space-y-6 mt-8">
      ...
      <div className="bg-gradient-to-r from-orange-500 to-orange-600 rounded-2xl p-8 text-white shadow-lg">
        <h2 className="text-3xl font-bold mb-2">Характеристики автомобиля</h2>
        <p className="text-orange-100">
          Всего опций: {standardOptions.length + choiceOptions.length}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-gradient-to-br from-green-50 to-white border-2 border-green-200 rounded-xl p-6 text-center">
          ...
          <p className="text-xs text-gray-600 mt-2">опций в комплекте</p>
        </div>
        ...
        {choiceOptions.length > 0 && (
          <div className="bg-gradient-to-br from-blue-50 to-white border-2 border-blue-200 rounded-xl p-6 text-center">
            ...
            <p className="text-xs text-gray-600 mt-2">доступно на выбор</p>
          </div>
        )}
      </div>
    </div>
