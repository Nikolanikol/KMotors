...
const WA_TEXT: Record<string, string> = {
  ru: "Здравствуйте! Хочу узнать об автомобилях из Кореи",
  en: "Hello! I'd like to know more about Korean cars",
  ko: "안녕하세요! 한국 자동차에 대해 알고 싶습니다",
  ka: "გამარჯობა! გთხოვ მომაწოდოთ ინფორმაცია კორეული ავტომობილების შესახებ",
  ar: "مرحباً! أريد معرفة المزيد عن السيارات الكورية",
};

const TG_SVG = (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 sm:w-6 sm:h-6 flex-shrink-0">
...
export default function MessengerButtons() {
  ...
  const waText = encodeURIComponent(WA_TEXT[lang] ?? WA_TEXT.ru);
  const waUrl = `https://wa.me/${WA_PHONE}?text=${waText}`;
  const tgUrl = "https://t.me/KMOTORS_form_bot?start=website";
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      ...
      <div className="hidden sm:flex flex-col gap-3">
        <FloatButton
          href={waUrl}
          label={WA_LABEL[lang] ?? WA_LABEL.ru}
          icon={WA_SVG}
          color="bg-[#25D366]"
        ...
        <FloatButton
          href={tgUrl}
          label={TG_LABEL[lang] ?? TG_LABEL.ru}
          icon={TG_SVG}
          color="bg-[#229ED9]"
      ...
      <div className="sm:hidden flex flex-col items-end gap-2">
        <div
          className={`flex-col gap-2 ${mobileOpen ? "flex" : "hidden"}`}
        >
          <FloatButton
            href={waUrl}
            label={WA_LABEL[lang] ?? WA_LABEL.ru}
        ...
        <button
          ...
          className={`p-3 rounded-full text-white shadow-lg transition-all duration-300 ${
            mobileOpen ? "bg-gray-600 hover:bg-gray-700 rotate-0" : "bg-[#F57C00] hover:bg-[#E65100]"
          }`}
