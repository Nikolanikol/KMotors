          ...
          <nav className="mt-8 flex items-center justify-center gap-2 flex-wrap" aria-label="Pagination">
            {pageNums.map((p, idx, arr) => (
              <span key={p} className="flex items-center gap-2">
                ...
                {p === page ? (
                  <span className="px-3 py-1.5 rounded-lg bg-[var(--pn-orange-deep)] bg-[image:var(--pn-fill)] text-white text-sm font-semibold shadow-lg shadow-[rgba(182,119,73,0.25)]">{p}</span>
                ) : (
                  <Link
