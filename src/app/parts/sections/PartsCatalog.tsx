...
async function fetchCatalogData() {
  ...
  const [
    ...
    supabase
      .from("parts_products")
      .select(
        "id, name, name_en, part_number, price, brand_id, category_id, subcategory_id, image_url, is_new, is_available"
      )
      .order("name"),
    supabase.from("parts_fitment").select("product_id, vehicle_model_id"),
    supabase.from("parts_vehicle_models").select("id, name, brand_id, slug").order("name"),
  ]);
