 ...
 *
 * Подписчику при отсутствии новинок не уходит НИЧЕГО — это штатно, но снаружи
 * неотличимо от мёртвого крона. Ровно так рассылка простояла с первого дня:
 * cron писал `not found` в лог, а признаков жизни от неё никто и не ждал.
 * Отчёт делает тишину наблюдаемой, не трогая то, что видит клиент.
 *
 * ⚠️ Уходит в TELEGRAM_WORK_CHAT_ID, то есть админу, и НИКОГДА подписчику — это
 * разные ветки, sendToChat пишет по chat_id самого подписчика. При суточном
 * расписании это одно сообщение в день; если расписание снова участят для
 * тестов, отчёт участится вместе с ним.
 *
 * Не бросает исключений и не влияет на результат прогона: упавший отчёт не
