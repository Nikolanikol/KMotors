...
Поиск в каталоге запчастей — отправка ЯВНАЯ, и URL там пишется мимо роутера

Строка поиска на /[lang]/parts (SearchBar в PartsCatalogClient.tsx — тот инпут, что внутри #catalog, не пилюля в Hero) раньше искала по дебаунсу 300 мс на каждый ввод. На артикуле «26300-35505» это до 11 запросов к /api/parts/products по 48 700 товарам за один осознанный поиск, и каждый из них тянул за собой ещё и router.push. Теперь запрос уходит только по кнопке «Найти», по Enter, по крестику очистки и по событию parts:hero-search (там пользователь уже нажал поиск — второго клика не просим). Дебаунса нет вообще; не возвращать.

Три вещи, которые в коде не видны и которые легко сломать обратно:

- q пишется через window.history.replaceState, а НЕ router.push. Серверный рендер /[lang]/parts от searchParams не зависит (их читает только generateMetadata ради noindex на отфильтрованных URL), а товары приходят отдельным клиентским fetch'ем в /api/parts/products. Значит router.push здесь — это чистая переплата: полный RSC-запрос всей страницы (Hero, каталог, About, ContactForm) в довесок к запросу данных. Фильтры, сортировка и пагинация НАМЕРЕННО остались на router.push: там URL — источник правды для выборки, и через него же работают back/forward. Отсюда параметр updateParams(updates, { replace }).
- updateParams строит параметры из window.location.search, а не из useSearchParams(). После replaceState хук обновляется не синхронно, и следующий клик по фильтру уносил бы q из адреса.
- Состояние ввода живёт ВНУТРИ SearchBar (memo), наружу уходит только отправленное значение. В корневом компоненте оно перерисовывало бы на каждую букву 24 карточки, сайдбар с фасетными счётчиками и облако категорий.

Отдельно про маунт: updateFilters всегда снимает page. Пока коммит запроса висел на эффекте без защиты от первого прогона, заход на /ru/parts?page=5 через 300 мс сам себя перекидывал на первую страницу и слал ложное событие search в аналитику при каждой загрузке. Любой новый эффект, дёргающий updateFilters, обязан пропускать первый прогон.
...
Секреты: два, не больше. POSTER_CRON_SECRET (заголовок x-poster-secret) закрывает /api/poster/run, /api/poster/parts/run, /api/rss-sync, /api/blog-generate и /api/subscriptions/run. SEO_CRON_SECRET (заголовок x-seo-secret) закрывает SEO-пайплайн. CRON_SECRET больше не используется нигде — он был задан только в README и ни разу в окружении.
...
ИСПРАВЛЕНО 29.07.2026: sitemap-parts/[page] сортировал .order("price_krw") без уникального тайбрейкера. Цена не уникальна (на 1000 строк 691 различная, до 15 товаров на одну), а ORDER BY без тайбрейкера не задаёт полного порядка — два одинаковых запроса за страницей 2 разошлись на 608 позиций из 1000, страницы 2 и 3 пересеклись. Так как каждый файл генерируется отдельно и кешируется на сутки, товар со «стыковой» ценой попадал либо в два файла, либо ни в один. Лечение — .order("id") вторым ключом; после правки пересечение 0, порядок воспроизводим. Тот же тайбрейкер превентивно добавлен в sitemap-fitment.xml (.order("parts_count"), 56% значений неуникальны) — там баг спящий, пока авто с parts_count >= 10 меньше 1000 и цикл не доходит до второй страницы. sitemap-blog.xml намеренно без тайбрейкера: пагинации нет, порядок внутри единственного файла безразличен.

Правило общее: любой .range()/.limit() по неуникальному ключу обязан иметь вторым ключом .order("id").

130 артикулов с несколькими строками — это НЕ дубли записей, не сливать

В parts_products 130 значений part_number встречаются больше одного раза, всего 308 «лишних» строк (0.6% каталога). Выглядит как грязь в данных, но это не она: одна физическая деталь занесена ПО СТРОКЕ НА КАЖДЫЙ СОВМЕСТИМЫЙ АВТОМОБИЛЬ. Пример — 319224H001 (картридж топливного фильтра, 13 строк: Sportage, Soul, Avante MD, Santa Fe…), P00BWSZO (галогенка H7, 90 строк на 90 машин, и сам номер похож на внутренний код поставщика, а не на OEM).

Различаются detail_url и product_no во всех 130 группах, названия в 110, цены в 79. Слияние уничтожит данные: в названиях сидит информация о машине, цены реально разные.

Следствия, которые надо просто знать:
- Канонический URL — артикул, поэтому все строки группы схлопываются в одну страницу; 308 строк собственной страницы не имеют. Код это учитывает: в [lang]/parts/[slug]/page.tsx стоит .limit(1), а НЕ .single() (тот падал с ошибкой). Какая именно строка покажется, формально не гарантировано, но замер 20 одинаковых запросов подряд дал один и тот же ряд — выборка по индексу, не сортировка с офсетом.
- В сайтмапе один и тот же <loc> повторяется. Дедупликацией внутри файла это НЕ чинится: в 79 группах из 130 цены разные, а файлы режутся по цене, значит копии разъезжаются по разным файлам. Понадобился бы DISTINCT ON (part_number), который через PostgREST не выражается — нужен view или RPC.

Настоящее лечение — то же самое, что и консолидация совместимости: одна строка на деталь, машины через part_vehicles. Отдельной задачи не заводить, это аргумент в пользу уже запланированной.
...
Страница проданной машины — /[lang]/catalog/[id] при data === null

Раньше тут стоял notFound(), и весь входящий поисковый трафик на проданные машины (сотни URL в индексе) попадал в пустую заглушку — при том, что посетитель тут самый квалифицированный из возможных, он искал конкретную модель. Теперь ветка рендерит SoldCar: снимок из cars_seen (фото, характеристики, перечёркнутая прежняя цена), кнопка подписки deep-link'ом в бота и мессенджеры с готовым текстом.

- ⚠️ noindex остаётся. Страница живёт ради людей, уже пришедших из индекса, а не ради ранжирования; вернём индексацию — вернутся сотни «дубликатов без канонического» в GSC. Выставляется независимо, в generateMetadata, и от рендера не зависит.
- Имя собирается ТОЛЬКО из английских полей снимка через normalizeBrand. Нет их (снимок из бэкфилла) — показываем нейтральный заголовок, а не хангыль.
- Снимка нет вовсе — блок похожих не рендерится, страница деградирует до «объявление снято» + подписка + каталог.
- Похожие подбираются src/lib/similarCars.ts: марка + модельная группа + год ±1 + цена ±15%, корейскими именами из снимка. Собственный recommend/{id} у Encar для этого не годится — он привязан к живому объявлению.

Подписки «пришлём похожие» (src/lib/savedSearches.ts, sql/036_saved_searches.sql)

Человек нажал кнопку на странице проданной машины → бот запоминает запрос → крон раз в сутки пишет ему, когда подходящее появилось. Смысл в том, что на проданную карточку приходят именно за тем, чего сейчас НЕТ в наличии.

Бот — существующий KMOTORS_form_bot, тот же TELEGRAM_BOT_TOKEN. Его /start уже публичный и обрабатывается ДО гейта isAdmin, рядом с разбором car_<id>; отдельный бот разнёс бы клиентский опыт по двум чатам. Метка sold_<encar_id> несёт всё нужное, поэтому ни токенов, ни промежуточного роута создания подписки нет.

Инварианты — все четыре найдены живыми тестами и в коде не видны:

- ⚠️ Новизна определяется по manage.firstAdvertisedDateTime из КАРТОЧКИ, а не по отсутствию Id в seen_ids. Выдача Encar сортируется по ModifiedDate и пересобирается, а забираем мы верхние FETCH_LIMIT из выборки, которая бывает в разы больше (замер: 373 совпадения при окне 60) — старое объявление свободно вплывает в окно и выглядит новым. В первом же тесте «новинкой» оказалась машина, поданная за полтора месяца до подписки. По ModifiedDate ориентироваться тем более нельзя: она обновляется при каждом переподнятии дилером.
- Даты Encar отдаёт без зоны, время корейское — разбирать как +09:00 явно, иначе Date.parse считает их UTC и сдвигает на 9 часов.
- FETCH_LIMIT общий для засева и крона, живёт в savedSearches.ts. Разойдутся — и на первом прогоне давно висящие объявления уедут человеку как новинки.
- seen_ids схлопывается (compactSeen), иначе копит дубли: замер на проде дал 111 записей при 60 уникальных. Оставляем ПОСЛЕДНЕЕ вхождение — срез идёт с конца.
- Названия машин берутся из карточки (manufacturerEnglishName / modelGroupEnglishName / gradeEnglishName + normalizeBrand), а НЕ из листинга: в листинге только корейские, и первая живая рассылка ушла на хангыле. Карточка и так запрашивается ради даты подачи, второго запроса не нужно.
- parse_mode HTML, имена приходят от Encar — экранировать < > &. Кривой тег роняет отправку целиком (Telegram отвечает ok:false).
- Апстрим не ответил (failed) — НЕ трогать ни seen_ids, ни last_sent_at: авария Encar молча съела бы новинки, зачислив их в «виденные» без отправки. По той же причине seen_ids пополняется только после УДАЧНОЙ отправки.
- Подписку не создаём, если Encar лёг: пустой засев = ложные новинки на первом прогоне.
- Отписка (кнопка unsub:<id>) сверяет chat_id — callback_data приходит от клиента, без сверки любой отписал бы чужую подписку по произвольному id. Обрабатывается до гейта isAdmin, как и /start. Массовая отписка — команда /stop.

Настройки (решение владельца): проверка раз в сутки, не чаще одного сообщения в сутки на человека, не больше пяти машин в сообщении. Одинаковые запросы дедуплицируются по строке query — из Encar тянутся один раз и раздаются веером.
...
- Layer 1, Cloudflare WAF (outside this repo, nothing here can read or change it): a custom rule blocks requests from Korea at the edge. The exception is a higher-priority Skip rule keyed on a cookie — expression `http.cookie contains "my_secret_key=<value>"`. The VALUE IS DELIBERATELY NOT IN THIS FILE: the GitHub repo is PUBLIC (verified via `gh repo view` — visibility PUBLIC), so anything committed here is world-readable forever, including git history and every fork. Keep it in .env / a password manager. To use it, set the cookie once in the browser for kmotors.shop, or pass `--cookie "my_secret_key=<value>"` to curl; no app code reads it.
- ⚠️ Область блока — ТОЛЬКО каталог авто, не весь сайт (замерено 29.07.2026 с корейского IP): 403 отдают `/{lang}/catalog` и `/{lang}/catalog/{id}`; `/{lang}`, `/{lang}/parts`, `/{lang}/blog`, `/{lang}/calculator`, `/sitemap.xml`, `/robots.txt` и `/api/*` открыты и проверяются без всякой cookie. Пришедший из Кореи 403 на карточке авто — это работающее правило, а не сломанный прод; не искать причину в приложении.
